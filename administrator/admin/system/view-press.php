  <?php
    ...
    if(isset($_GET['id'])){
        ...
        if($update){
            echo "<script>alert('Image Disabled successfully'); window.location='view-press';</script>";
            // header("Location: view-press");
        }
    }
    ...
    if(isset($_GET['did'])){
        ...
        $update =  $crud->Update("press",$data,"`id`='$id'");
        if($update){
            echo "<script>alert('Image enabled successfully'); window.location='view-press';</script>";
            // header("Location: view-press");
        }
    }
?>
                                        ...
                                        try{
                                            ...
                                            if ($con) {
                                             //echo "<script>alert('$id')</script>";
                                                $news_retr_sql = "select *  from `press` where `id`<> 0";
                                          ...
                                          <tr>
                                              ...
                                              <td class="text-center">
                                                <?php
                                                    $catid = $row['category'];
                                                    $category_sql = "select *  from `press_category` where `id`='$catid'";
                                              ...
                                              <th>
                                                  <?php if($row['status'] == 1){ ?>
                                                  <a href="view-press?id=<?php echo $row['id']?>"
                                                      class="btn btn-success">Block</a>
                                                  <?php }else if($row['status'] == 0){ ?>
                                                  <a href="view-press?did=<?php echo $row['id']?>"
